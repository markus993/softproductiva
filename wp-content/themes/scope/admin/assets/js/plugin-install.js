jQuery(document).ready(function($) {
    'use strict';
    var this_obj = scope_companion_install;
    ...
    $(document).on('click', '.action-watch', function(event) {
        ...
        if(action_id){
            $.ajax({
                ...
                type: 'POST',
                data: {action: 'scope_update_rec_acts', action_id:action_id},
                success:function(data) {
    ...
    $(document).on('click', '.themefarmer-customizer-notification-dismiss', function(event) {
        ...
        var sdata = {};
        if(slug){
            sdata.action = 'scope_hide_customizer_notice';
            sdata.scope_plugin_slug = slug;
        }else{
            sdata.action = 'scope_hide_customizer_companion_notice';
        }
